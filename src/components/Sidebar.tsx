...
export default function Sidebar({ aberto, fecharMenu }: SidebarProps) {
  return (
    <>
      {/* Overlay escuro no mobile (só aparece se o menu estiver aberto) */}
      {aberto && (
        <div 
          onClick={fecharMenu}
      ...
      {/* A Sidebar em si: Desliza no mobile, fixa no PC */}
      <aside className={`fixed top-0 left-0 h-screen w-[260px] md:w-[104px] bg-white dark:bg-stone-900 border-r border-stone-200 dark:border-stone-800 z-50 flex flex-col md:items-center py-8 transition-transform duration-300 ease-in-out ${aberto ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}>
        ...
        <nav className="flex flex-col gap-2 md:gap-6 w-full px-4 md:px-0 md:items-center">
          <Link href="/dashboard" className="flex items-center gap-4 p-3 rounded-xl bg-stone-100 dark:bg-stone-800 text-[#A67B5B] transition-colors" title="Dashboard">
            <LayoutDashboard size={24} className="shrink-0" />
            <span className="md:hidden font-medium">Dashboard</span>
          </Link>
          <Link href="#" className="flex items-center gap-4 p-3 rounded-xl text-stone-500 hover:text-[#A67B5B] hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors" title="Contratos & Acordos">
            <FileSignature size={24} className="shrink-0" />
            <span className="md:hidden font-medium">Contratos</span>
          </Link>
          <Link href="#" className="flex items-center gap-4 p-3 rounded-xl text-stone-500 hover:text-[#A67B5B] hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors" title="Financeiro">
            <Wallet size={24} className="shrink-0" />
            <span className="md:hidden font-medium">Financeiro</span>
          </Link>
          <Link href="#" className="flex items-center gap-4 p-3 rounded-xl text-stone-500 hover:text-[#A67B5B] hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors" title="Locadores / Credores">
        ...
        <div className="mt-auto px-4 md:px-0 w-full md:flex md:justify-center">
          <button className="flex items-center gap-4 p-3 w-full md:w-auto rounded-xl text-stone-500 hover:text-stone-800 dark:hover:text-stone-200 transition-colors">
