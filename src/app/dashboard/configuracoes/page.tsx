...
import { Settings, User, Briefcase, Wallet, Save, Loader2, ShieldCheck, Building, MapPin, Link as LinkIcon, Image as ImageIcon, Camera, Network } from "lucide-react";
...
export default function ConfiguracoesPage() {
  ...
  const handleSalvar = async (e: React.FormEvent) => {
    ...
    try {
      // Busca o ID de forma segura (sem quebrar se não existir)
      const { data: perfis } = await supabase.from("perfil_global").select("id").limit(1);
      const perfilExistente = perfis && perfis.length > 0 ? perfis[0] : null;
      ...
      if (perfilExistente) {
        ...
        const { error } = await supabase.from("perfil_global").update(dadosParaSalvar).eq("id", perfilExistente.id);
        if (error) throw error;
      } else {
        // Cria pela primeira vez
        const { error } = await supabase.from("perfil_global").insert([dadosParaSalvar]);
        if (error) throw error;
      }

      await atualizarPerfil();
      toast.success("Identidade sincronizada com sucesso! ✨", { id: toastId });
    } catch (error: any) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        {/* O Crachá Premium (Otimizado para textos longos) */}
        <div className="lg:col-span-1 sticky top-8">
          <div className="relative group w-full aspect-[3/4] rounded-3xl bg-gradient-to-b from-stone-900 via-stone-800 to-stone-950 shadow-2xl overflow-hidden border border-stone-700/50 flex flex-col transition-transform duration-500 hover:scale-[1.02]">
            ...
            <div className="p-8 pt-12 flex-1 flex flex-col relative z-10">
              ...
              {/* Header do Crachá (Agora o texto quebra a linha se for grande) */}
              <div className="flex items-start gap-3 mb-8">
                <div 
                  onClick={() => logoInputRef.current?.click()}
                  className="group/logo shrink-0 cursor-pointer relative h-12 w-12 bg-white/10 backdrop-blur-md rounded-xl flex items-center justify-center overflow-hidden border border-white/10 hover:border-white/30 transition-all mt-1"
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] uppercase tracking-wider text-emerald-400/80 font-bold mb-0.5">Vínculo Ativo</p>
                  <p className="text-sm font-bold text-white leading-tight line-clamp-2" title={empresaNome}>{empresaNome || "Nome da Instituição"}</p>
                  {setor && <p className="text-xs text-stone-400 mt-1 line-clamp-1">{setor}</p>}
                </div>
              </div>
              ...
              <div className="text-center space-y-2 mt-2">
                <h3 className="text-2xl font-black text-white leading-tight">{nome || "Seu Nome"}</h3>
                <p className="text-[#A67B5B] font-bold tracking-wide uppercase text-sm line-clamp-1">{cargo || "Seu Cargo"}</p>
              </div>

              {/* Rodapé do Crachá */}
              <div className="mt-auto pt-6 border-t border-white/10 flex justify-between items-end text-xs text-stone-400">
                <div className="flex items-center gap-1.5 min-w-0 pr-2">
                  <MapPin size={14} className="text-[#A67B5B] shrink-0" />
                  <span className="truncate">{localTrabalho || "Local não definido"}</span>
                </div>
                <ShieldCheck size={18} className="text-emerald-500 opacity-80 shrink-0" />
              </div>
            </div>
          </div>
        </div>

        {/* Coluna Direita: Formulários Repensados e Espaçados */}
          ...
          <form id="form-perfil" onSubmit={handleSalvar} className="p-8 rounded-3xl bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 shadow-sm space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2 group">
                  <label className="text-sm font-semibold text-stone-500 flex items-center gap-1"><ImageIcon size={14}/> Link da Foto (Opcional)</label>
            ...
            <div className="space-y-4">
              <h4 className="text-lg font-bold text-stone-900 dark:text-stone-100 border-b border-stone-100 dark:border-stone-800 pb-2 flex items-center gap-2">
                <Building size={18} className="text-[#A67B5B]" /> Estrutura do Vínculo
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                
                {/* Linha 1: Ocupa tudo para dar espaço a nomes grandes */}
                <div className="space-y-2 group md:col-span-2">
                  <label className="text-sm font-semibold text-stone-500">Órgão / Empresa / Instituição</label>
                  <input required value={empresaNome} onChange={(e) => setEmpresaNome(e.target.value)} type="text" placeholder="Ex: Secretaria de Estado da Saúde do Maranhão" className="w-full bg-stone-50 dark:bg-stone-950 border border-stone-200 dark:border-stone-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#A67B5B]/50 transition-all" />
                </div>

                {/* Linha 2: Cargo e Setor lado a lado */}
                <div className="space-y-2 group">
                ...
                <div className="space-y-2 group">
                  <label className="text-sm font-semibold text-stone-500 flex items-center gap-1"><Network size={14}/> Setor / Lotação</label>
                  <input value={setor} onChange={(e) => setSetor(e.target.value)} type="text" placeholder="Ex: Gabinete de Inovação" className="w-full bg-stone-50 dark:bg-stone-950 border border-stone-200 dark:border-stone-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#A67B5B]/50 transition-all" />
                </div>

                {/* Linha 3: Local e Gestor */}
                <div className="space-y-2 group">
                  <label className="text-sm font-semibold text-stone-500">Local (Base de Atuação)</label>
                ...
                <div className="space-y-2 group">
                  ...
                  <input value={gestor} onChange={(e) => setGestor(e.target.value)} type="text" placeholder="Nome do superior direto" className="w-full bg-stone-50 dark:bg-stone-950 border border-stone-200 dark:border-stone-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#A67B5B]/50 transition-all" />
                </div>

                {/* Linha 4: Link da Logo (Ocupa tudo) */}
                <div className="space-y-2 group md:col-span-2">
                  <label className="text-sm font-semibold text-stone-500 flex items-center gap-1"><LinkIcon size={14}/> Link da Logo (Alternativa ao clique)</label>
                  <input value={empresaLogoUrl} onChange={(e) => setEmpresaLogoUrl(e.target.value)} type="text" placeholder="Cole uma URL caso prefira não enviar o arquivo" className="w-full bg-stone-50 dark:bg-stone-950 border border-stone-200 dark:border-stone-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[#A67B5B]/50 transition-all text-stone-400 truncate" />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <h4 className="text-lg font-bold text-stone-900 dark:text-stone-100 border-b border-stone-100 dark:border-stone-800 pb-2 flex items-center gap-2">
                <Wallet size={18} className="text-[#A67B5B]" /> Base de Cálculo Financeiro
              </h4>
              <div className="space-y-2 group w-full md:w-1/2">
                <label className="text-sm font-semibold text-stone-500">Renda Base Atual (R$)</label>
            ...
            <div className="pt-4 flex justify-end">
              <button type="submit" disabled={salvando} className="flex items-center gap-2 px-8 py-4 bg-stone-900 dark:bg-stone-100 hover:bg-stone-800 dark:hover:bg-white text-stone-50 dark:text-stone-900 rounded-xl font-bold shadow-lg hover:shadow-xl transition-all hover:-translate-y-1 disabled:opacity-70 disabled:hover:-translate-y-0">
                ...
                <span>{salvando ? "Emitindo..." : "Salvar Identidade"}</span>
              </button>
            </div>
